...
int quantize_lsp(const float *x, const float *codebook1, const float *codebook2, const float *codebook3, int nb_entries,
                 float *xq, int ndim) {
  int i, n1, n2, n3;
...
void update_multi(float *data, int nb_vectors, float *codebook, int nb_entries, int ndim, int sign) {
  ...
  fprintf(stderr, "%d %d %d %d %d %d %d %d ", idcount[0], idcount[1], idcount[2], idcount[3], idcount[4], idcount[5],
          idcount[6], idcount[7]);
  fprintf(stderr, "| %f / %d, min = %d, small=%d\n", 1. / w2, nb_entries, min_count, small);
  RESTORE_STACK;
}
...
#else
  for (i = 0; i < MULTI * ndim; i++)
    codebook[i] = .01 * (rand() / (float)RAND_MAX - .5);
#endif
  e = MULTI;
  for (j = 0; j < 10; j++)
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nb_vectors - 4; i++) {
    for (j = 0; j < ndim; j++)
      multi_data[MULTI * i * ndim + j] =
          data[(i + 1) * ndim + j] - .5 * (qdata[i * ndim + j] + qdata[(i + 2) * ndim + j]);
    for (j = 0; j < ndim; j++)
      multi_data[(MULTI * i + 1) * ndim + j] =
          data[(i + 1) * ndim + j] - .5 * (qdata[i * ndim + j] + qdata[(i + 2) * ndim + j]);
    for (j = 0; j < ndim; j++)
  ...
  for (i = 0; i < nb_vectors - 4; i++) {
    for (j = 0; j < ndim; j++)
      multi_data2[MULTI * i * ndim + j] =
          data[(i + 2) * ndim + j] - .5 * (qdata[i * ndim + j] + qdata[(i + 4) * ndim + j]);
    for (j = 0; j < ndim; j++)
      multi_data2[(MULTI * i + 1) * ndim + j] =
          data[(i + 2) * ndim + j] - .5 * (qdata[i * ndim + j] + qdata[(i + 4) * ndim + j]);
    for (j = 0; j < ndim; j++)
